...
double calculate_chi2(POINTING *p, int N_bins, int lower_ind, int upper_ind){
    ...
    for(i = lower_ind; i < upper_ind; i++){
        ...
        for(j = 0; j < N_bins; j++){
            ...
            if( p[i].rbin[j].sigma2 == 0.0 ){
                p[i].rbin[j].chi2_local = 0.0;
                continue;
            }

            p[i].rbin[j].chi2_local = ( ( p[i].rbin[j].corr - 1.0 ) *
                ( p[i].rbin[j].corr - 1.0 ) / p[i].rbin[j].sigma2 );

            chi2 += p[i].rbin[j].chi2_local;
        }
    }
