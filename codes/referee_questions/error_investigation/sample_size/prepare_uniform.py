...
def main():
    ...
    ratio           = int(args_array[1])
    out_path        = args_array[2]
    ...
    for p in todo_list:
        ...
        Ntot = p.N_star * ratio # total number of random points to generate
        ...
        # pickle output
        output_filename = './' + out_path + '/uniform_' + p.ID + '.dat'
        output_file = open(output_filename, "wb")
        pickle.dump(random_sample, output_file)
        output_file.close()

        # output ascii format
        output_filename = './' + out_path + '/uniform_' + p.ID + '.ascii.dat'
