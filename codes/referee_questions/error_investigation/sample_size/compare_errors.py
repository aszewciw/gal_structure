...
ax.set_xlabel('r (kpc)')
ax.set_ylabel(r'$\displaystyle\frac{\frac{\delta_{MM}^2}{MM^2} + \frac{\delta_{DD}^2}{DD^2}}{\frac{\delta_{DD}^2}{DD^2}}$')
ax.set_title('Jackknife Error Comparison (100 * density)')
ax.set_xscale('log')
...
for j in range(len(todo_list)):
    ...
    uni_jk_file = out_path + 'uniform_' + p.ID + '_jk_error.dat'
    uni_jk_err  = np.genfromtxt(uni_jk_file, unpack=True, usecols=[7])
    dat_jk_file = jk_data_dir + 'star_' + p.ID + '_jk_error.dat'
    dat_jk_err  = np.genfromtxt(dat_jk_file, unpack=True, usecols=[7])
    frac        = np.ones(len(uni_jk_err))

    err_dd_2    = dat_jk_err ** 2
    err_mm_2    = uni_jk_err ** 2
    err2_temp   = err_dd_2 + err_mm_2
    for i in range(len(frac)):
        if err_dd_2[i] > 0.0 and err_mm_2[i] > 0.0:
            # frac[i]      = err_dd_2[i] / err2_temp[i]
            frac[i]      = err2_temp[i] / err_dd_2[i]
            # frac[i] = err2_temp[i] / err_mm_2[i]
            errors[j, i] = frac[i]
            N_errs[i]    += 1


    ax.plot(binplt, frac, '0.75', zorder = -42)
    # plt.axis([0.005, 2, 1, 2])
