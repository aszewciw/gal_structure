...
'''
Reads in corr_list containing info about cleaned SEGUE data.
...
def main():
    ...
    sys.stderr.write('{} line-of-sight to generate..\n'.format(len(corr_list)))

    # generate random sample and star sample for individual plates
    for p in corr_list:

        # a progress indicator
        if corr_list.index(p) % 10 == 0:
            sys.stderr.write('Generating #{} of {} ..\n'
                             .format(corr_list.index(p), len(corr_list)))

        if p.N_star == 0:
            sys.stderr.write('Error: Empty star list. \n')
            continue

        mock_file = data_dir + 'mock_full_' + p.ID + '.xyz.dat'
        temp = np.genfromtxt(mock_file, skip_header=1)
        Ntot = star_factor * len(temp)
        ...
        # # pickle output
        # output_filename = data_dir + 'uniform_10mock_' + p.ID + '.dat'
        # output_file     = open(output_filename, "wb")
        # pickle.dump(random_sample, output_file)
        # output_file.close()

        # output ascii format
        output_filename = data_dir + 'uniform_10mock_' + p.ID + '.ascii.dat'
        ...
        # output xyzw
        output_filename = data_dir + 'uniform_10mock_' + p.ID + '.xyz.dat'
        output_file = open(output_filename, "w")
        # first output the total number of points
        output_file.write('{}\n'.format(len(random_sample)))
        for i in random_sample:
