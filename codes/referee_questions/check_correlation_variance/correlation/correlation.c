...
int main(int argc, char **argv){
  ...
  /* first read in number of points */
  fscanf(file2, "%d", &N);
  fprintf(stderr, "Read %d particles in uniform.\n", &N);
  data2.N = N;
  ...
  for(r_log = rmin_log; r_log < rmax_log; r_log += dr_log){
    ...
    // NOTE: I should have changed the line below for the LS method
    // fprintf(stdout, "%lf\t%lf\t%lf\t%le\t%le\t%le\t%le\t%le\t%le\n",
	   //  r1, correlation, pow(10.0, dr_log),
	   //  DD, DD_norm, RR, RR_norm, DR, DR_norm);
