...
/* Calculate DD/RR -- must only be done once */
void calculate_DD_RR(POINTING *p, int N_bins, int lower_ind, int upper_ind){
    ...
    for(i=lower_ind; i<upper_ind; i++){
        ...
        for(j=0; j<N_bins; j++){
            ...
            if( p[i].rbin[j].DD == 0.0 || p[i].rbin[j].DD == 0.0){
                p[i].rbin[j].DD_RR = 0.0;
                continue;
            }

            p[i].rbin[j].DD_RR = p[i].rbin[j].DD / p[i].rbin[j].RR;
        }
    }
}
...
/* Calculate MM/RR and DD/RR - MM/RR for each bin in each l.o.s. */
void calculate_correlation(POINTING *p, int N_bins, int lower_ind, int upper_ind){
    ...
    for(i = lower_ind; i < upper_ind; i++){
        ...
        for(j = 0; j < N_bins; j++){
            ...
            if( p[i].rbin[j].DD == 0.0 || p[i].rbin[j].MM == 0.0 || p[i].rbin[j].RR == 0.0 ){
                p[i].rbin[j].MM_RR = 0.0;
                p[i].rbin[j].diff = 0.0;
                continue;
            }
            p[i].rbin[j].MM_RR = p[i].rbin[j].MM / p[i].rbin[j].RR;
            p[i].rbin[j].diff = p[i].rbin[j].DD_RR - p[i].rbin[j].MM_RR;
        }
    }
}
...
/* I don't know how to calculate this when errors are correlated */
...
int degrees_of_freedom(POINTING *p, int N_bins, int lower_ind, int upper_ind){
    ...
    for(i = lower_ind; i < upper_ind; i++){
        ...
        for(j = 0; j < N_bins; j++){
            ...
            if( p[i].rbin[j].DD_RR == 0.0 ) continue;
