...
typedef struct {
  ...
  double corr; // DD/MM
  double DD_err_jk; // TRUE FRACTIONAL ERROR (NOT JK)
...
/* MCMC functions */
void set_weights(STEP_DATA params, POINTING *p, int lower_ind,
  int upper_ind);
...
void calculate_correlation(POINTING *p, int N_bins, int lower_ind,
  int upper_ind);
int degrees_of_freedom(POINTING *p, int N_bins, int lower_ind, int
  upper_ind);
