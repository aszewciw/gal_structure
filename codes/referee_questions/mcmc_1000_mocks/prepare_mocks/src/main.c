...
int main( int argc, char **argv ){
    ...
    if (argc != 3){
        fprintf( stderr, "Error. Usage: %s num_stars mock_num\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    fprintf(stderr, "On mock number %d of 1000\n", mock_num);
    ...
    while(loop_flag==0){
        ...
        separate_sample(plist, thin, N_plist, thin_params.N_stars, mock_num);
        separate_sample(plist, thick, N_plist, thick_params.N_stars, mock_num);
