...
def main():
    ...
    for p in todo_list:
        ...
        in_file = qq_dir + 'uniform_' + p.ID + '.ascii.dat'
        if not os.path.isfile(in_file):
            sys.stderr.write('Error: ' + in_file + ' does not exist.\n')
            continue

        x,y,z = np.genfromtxt(in_file, unpack=True, usecols=[7,8,9], skip_header=1)
        xy    = np.column_stack((x,y))
        xyz   = np.column_stack((xy,z))

        outfile = qq_dir + 'uniform_' + p.ID + '.xyz.dat'
        np.savetxt(outfile, xyz)
        N_stars = str(len(xyz))
