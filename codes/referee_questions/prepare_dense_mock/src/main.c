...
int main( int argc, char **argv ){
    ...
    /* how many times as many stars we want sample to have, compared to SEGUE */
    int star_factor;
    sscanf(argv[1], "%lu", &N_stars);
    fprintf(stderr, "%lu stars per temporary galaxy.\n", N_stars);

    sscanf(argv[2], "%d", &star_factor);
    fprintf(stderr, "Mock will have %d times as many stars as each SEGUE l.o.s.\n", star_factor);
    ...
    int N_data;             // # of stars in current SEGUE l.o.s.
    int N_desired;          // desired number of stars in current l.o.s.
