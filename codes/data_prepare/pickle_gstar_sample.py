...
def main():
    ...
    sys.stderr.write('Reading from input file {} ...\n'.format(input_filename))         #Why this instead of print?

    for line in file(input_filename):
        if line.lstrip().startswith('#'):                                         #This doesn't seem necessary for me because genfromtxt already ignores these lines
            continue
        s = Star()                                                     #See config. He imports the class Star and adds all of the the values below

        record = line.split()                                                                                       #Don't really get why this line is here
        # add an empty element at the beginning of the list,
        # so that the following index matches the column number.
        record.insert(0, '')

        s.sspecobjid = record[1]                                                                                #I think they are floats by default
        s.sobjid  = record[2]
        s.spplate = record[3]
        s.rv = float(record[4])
    ...
    for s in star_list:
        ...
        if (s.average_estimated_distance > 0
            and (s.g0 - s.r0) > 0.48
            and (s.g0 - s.r0) < 0.55
            and s.r0 > 15.0                                   #I think my limits were different here. This is probably why I counted more useable beams
            and s.r0 < 19.0):
    ...
    pickle.dump(gstar_list, output_file)                                                                            #Need to learn pickling
    sys.stderr.write('Pickle output to file {} .\n'.format(output_filename))

    output_file.close()


    # output_filename = './test_gal.dat'
    # output_file = open(output_filename, 'w')
    # for s in star_list:
    #     output_file.write("{}\t{}\t{}\t{}\n"
    #                       .format(s.galactic_l, s.galactic_b, s.galactic_l_deg, s.galactic_b_deg))
