#!/usr/bin/env python
'''
Reads in each pointing from SEGUE and generates a uniform sample
with the same geometry and same number. The points are then to be
weighted to test if my mcmc is working.
'''
from config import *
...
def main():
    ...
    for p in todo_list:
        ...
        # output
        output_filename = mcmcdata_dir + 'random_' + p.ID + '.dat'
        output_file = open(output_filename, "wb")
        pickle.dump(random_sample, output_file)
        output_file.close()

        # output ascii format
        output_filename = mcmcdata_dir + 'random_' + p.ID + '.xyzw.dat'
