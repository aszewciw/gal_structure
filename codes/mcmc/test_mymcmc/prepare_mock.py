...
def gal_weights(Z, R, a, z_thick, r_thick, z_thin, r_thin):
    ...
    '''
    Assign weights for each point
    '''

    weight = ( ( ( math.cosh(Z / 2 / z_thin) ) ** (-2) )
        * math.exp(-R / r_thin) +
        a * ( ( math.cosh(Z / 2 / z_thick) ) ** (-2) )
        * math.exp(-R / r_thick) )
...
def main():
    ...
    sys.stderr.write('Generating mock samples..\n')
    ...
    for p in todo_list:
        ...
        Ntot = p.N_star # total number of random points to generate
        ...
        # set weight based on best fit parameters
        for i in random_sample:
            i.weight = gal_weights(i.galactic_Z, i.galactic_R,
                a, z_thick, r_thick, z_thin, r_thin)

        # pickle output
        output_filename = mock_dir + 'mock_' + p.ID + '.dat'
        output_file = open(output_filename, "wb")
        pickle.dump(random_sample, output_file)
        output_file.close()

        # output ascii format
        output_filename = mock_dir + 'mock_' + p.ID + '.ascii.dat'
    ...
    sys.stderr.write('Done. Mock samples output to {} .\n\n'.format(mock_dir))
