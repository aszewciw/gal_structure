...
def main():
    ...
    for p in todo_list:
        ...
        # uni_jk_file = qiqi_dir + 'uniform_' + ID + '_jk_error.dat'
        # uni_jk_err  = np.genfromtxt(uni_jk_file, unpack=True, usecols=[7])
        # dat_jk_file = qiqi_dir + 'star_' + ID + '_jk_error.dat'
        # dat_jk_err  = np.genfromtxt(dat_jk_file, unpack=True, usecols=[7])
        uni_jk_file = err_dir + 'uniform_' + ID + '_frac_error.dat'
        uni_jk_err  = np.genfromtxt(uni_jk_file)
        dat_jk_file = err_dir + 'star_' + ID + '_frac_error.dat'
        dat_jk_err  = np.genfromtxt(dat_jk_file)
        ...
        for j in range(Nbins):
            ...
            model_file      = pairs_dir + 'pairs_' + ID + '.bin_' + str(j + 1) + '.dat'

            MODEL[los][BIN]['ind1'], MODEL[los][BIN]['ind2'] = np.genfromtxt(
                model_file, dtype=None, unpack=True, skip_header=1)
